...
struct pginfo {
    ...
    /* <eric> this is not needed since we're no longer doing power of two
    u_short		shift;	/* How far to shift for this size chunks */
    u_short		free;	/* How many free chunks */
    u_short		total;	/* How many chunk */
    /* <eric> not needed because we changed bitmap operations to reap-like operations
...
/* <eric> start */
/* <eric> granularity of size classes */
#if !defined(malloc_sizedistance)
#define malloc_sizedistance		8U
#endif

#if malloc_sizedistance % 8U || malloc_maxsize % malloc_sizedistance
#error "bad choice of malloc_sizedistance"
#endif

/* <eric> number of size classes */
#if !defined(malloc_sizeclasses)
#define malloc_sizeclasses		(malloc_maxsize / malloc_sizedistance)
#endif

#if (malloc_maxsize / malloc_sizedistance != malloc_sizeclasses)
#error "malloc_maxsize / malloc_sizedistance != malloc_sizeclasses"
#endif
/* <eric> end */
...
malloc_init(void)
{
    ...
    page_dir = (struct pginfo **) MMAP(malloc_pagesize);	/* <eric> this initial size should still be OK */
     ...
     */
	/* <eric> now the number of such buckets should be malloc_sizeclasses, 
	   these buckets (which should occupy less than malloc_pagesize) are stored at the front of the page_dir
	*/
    malloc_origo = ((u_long)pageround((u_long)sbrk(0))) >> malloc_pageshift;
    /* <eric> malloc_origo -= malloc_pageshift; */
	malloc_origo -= malloc_sizeclasses;		/* <eric> */

    malloc_ninfo = malloc_pagesize / sizeof *page_dir;
	if (malloc_ninfo <= malloc_sizeclasses)
		wrterror("initial size of page_dir too small\n");	/* <eric> since we now need more buckets, check if the page_dir is big enough */
...
static __inline int
/* <eric> malloc_make_chunks(int bits) */
malloc_make_chunks(int chunksize)	/* <eric> the input is now the chunk size */
{
    struct  pginfo *bp;
    void *pp;
    int l;
    ...
    l = sizeof(struct pginfo);	/* <eric> */

    /* Don't waste more than two chunks on this */
    /* <eric> if ((1<<(bits)) <= l+l) { */
    if (chunksize <= l+l) {		/* <eric> */
	bp = (struct  pginfo *)pp;
    } else {
	...
	/* <eric> start */
	bp->size = chunksize;
	bp->total = bp->free = malloc_pagesize / chunksize;
	bp->page = pp;
	bp->bumped = 0;
	bp->cur_ptr = bp->page;
	bp->free_list = NULL;
	...
	/* Mark the ones we stole for ourselves */
		while (l > 0) {
			bp->free--;
			bp->total--;
			bp->cur_ptr += chunksize;	/* <eric> move the start of the allocatable region */
			l -= chunksize;		/* <eric> */
		}
    }
	...
	*/
	...
	/* <eric> start */
    bp->next = page_dir[chunksize / malloc_sizedistance - 1];
    page_dir[chunksize / malloc_sizedistance - 1] = bp;
	/* <eric> end */
...
malloc_bytes(size_t size)
{
	...
	j++;
	*/
	j = (size - 1) / malloc_sizedistance;		/* <eric> find the right size class */

    /* If it's empty, make a page more of that size chunks */
    /* <eric> if (page_dir[j] == NULL && !malloc_make_chunks(j)) */
    if (page_dir[j] == NULL && !malloc_make_chunks((j + 1) * malloc_sizedistance))	/* <eric> */
	return (NULL);

    bp = page_dir[j];

	u_char *ptr;
	if (bp->bumped < bp->total) {	/* <eric> we can still do pointer-bumping */
		ptr = bp->cur_ptr;
		bp->bumped++;
		bp->cur_ptr += bp->size;
	}
	else {		/* <eric> otherwise we use the free list */
		ptr = (u_char *)bp->free_list;
		bp->free_list = (size_t *)*bp->free_list;
	}
...
irealloc(void *ptr, size_t size)
{
    ...
    /* <eric> if (index < malloc_pageshift) { */
	if (index < malloc_sizeclasses) {	/* <eric> */
	...
	/* <eric> if (((u_long)ptr & ((*mp)->size-1))) { */
    if (((u_long)ptr & malloc_pagemask) % (*mp)->size) {	/* <eric> */
...
free_bytes(void *ptr, u_long index, struct pginfo *info)
{
    ...
    /* <eric> i = ((u_long)ptr & malloc_pagemask) >> info->shift; */
    i = ((u_long)ptr & malloc_pagemask) / info->size; /* <eric> */

#ifdef MALLOC_SANITY	/* <eric> */
    /* <eric> if (((u_long)ptr & (info->size-1))) { */
    if (((u_long)ptr & malloc_pagemask) % info->size) {	/* <eric> */
	wrtwarning("modified (chunk-) pointer\n");
	return;
    }
#endif	/* <eric> MALLOC_SANITY */
	...
	*(size_t **)ptr = info->free_list;
	info->free_list = ptr;
    info->free++;

    /* <eric> mp = page_dir + info->shift; */
    mp = page_dir + info->size / malloc_sizedistance - 1;	/* <eric> */
